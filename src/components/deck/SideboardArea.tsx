...
export const SideboardArea = ({
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      if (dropTarget.getAttribute("data-deck-area") === "side") {
        onCardDrop(card, "side");
      }
    } catch (error) {
  ...
  // Create empty slots to fill the sideboard to 15 cards
  const emptySlotsNeeded = Math.max(0, 15 - cardCount);
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-lg font-bold">Sideboard</h2>
        <span className="text-sm">{cardCount} / 15 cards</span>
      </div>
